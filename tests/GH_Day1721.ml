...
let czy_cykliczne l =
  match try topol l with Cykliczne -> [] with
  | [] -> true
  | _ -> false
...
let test input output =
  let rec loop a b f = function
    ...
    | h :: t ->
        if f
        then if h = b then true else loop a b f t
        else if h = a
        then loop a b true t
        else loop a b f t
  and pom i a = function
    ...
    | h :: t -> (
        match loop a h false output with
        | true -> pom i a t
        | false -> false)
  in
...
let d =
  [
    "zolty", [ "niebieski"; "bialy"; "czarny" ];
    "bialy", [ "czarny" ];
    "czarny", [];
    "czerwony", [ "zielony"; "zolty"; "niebieski"; "czarny" ];
  ]

let e = [ 1, [ 2; 5; 8; 3 ]; 5, [ 8; 6; 4; 7 ]; 7, [ 6; 9; 2 ]; 8, [ 6; 9; 3 ] ]

let _ =
  assert (czy_cykliczne a);

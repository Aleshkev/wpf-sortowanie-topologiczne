...
let isOk input output =
  let rec where a = function
    | [] -> []
    | h :: t -> if h = a then h :: t else where a t
  in
  let rec length used = function
    | [] -> List.length used
    | (h, l1) :: t ->
        let newOne used a = if where a used = [] then a :: used else used in
        length (newOne (List.fold_left newOne used l1) h) t
  in
  let size = length [] input in
  let rec find acc wh = function
    | [] -> acc
    | h :: t -> acc && where h wh <> [] && find acc wh t
  in
  let rec pom acc = function
    | [] -> acc
    | (a, l) :: t -> acc && find acc (where a output) l && pom acc t
  in
  size = List.length output && pom true input
...
let _ = if !zle <> 0 then Printf.printf "\nBlednych testow: %d...\n" !zle

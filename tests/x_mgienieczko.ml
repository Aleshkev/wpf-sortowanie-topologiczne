...
let test graph order =
  ...
  let check_one (v, l) =
    List.iter
      (fun u ->
        if Hashtbl.find hashtbl v > Hashtbl.find hashtbl u then raise WA)
      l
  in
  try
    List.iter check_one graph;
    true
  with WA -> false
...
let g = [ 1, [ 2; 3 ]; 2, [ 4 ]; 3, [ 4 ]; 4, [ 5; 6 ]; 5, [ 7 ]; 6, [ 7 ] ];;
...
let g = [ 10.001, [ 10.002 ]; 10.002, [ 10.001 ] ];;
...
let g = [ 1, [ 2 ]; 2, [ 3 ]; 3, [ 4; 5; 3 ] ];;

assert (test_cyclic g)

let g = [ "pole", [ "pole"; "lyse"; "pole" ] ];;

assert (test_cyclic g)

let g = [ "tu", [ "tudu" ]; "tudu", [ "tudu"; "tudu"; "tudu" ] ];;
...
let g = [ "Polska", [ "Niemcy" ]; "Niemcy", [ "Polska" ] ];;

assert (test_cyclic g)

let g = [ 1, [ 2 ]; 3, [ 4 ]; 4, [ 1 ]; 5, [ 6 ]; 6, [ 3 ]; 2, [ 5 ] ];;

assert (test_cyclic g)

let g = [ 1, [ 2 ]; 3, [ 4 ]; 4, [ 1 ]; 5, [ 6 ]; 6, [ 3 ]; 1, [ 5 ] ];;

assert (test_cyclic g)

let g = [ 1, [ 2 ]; 3, [ 4 ]; 4, [ 1 ]; 5, [ 6 ]; 6, [ 3 ]; 2, [ 6 ] ];;

assert (test_cyclic g)

let g = [ 1, [ 2 ]; 3, [ 4 ]; 4, [ 1 ]; 5, [ 6 ]; 6, [ 3 ]; 1, [ 6 ] ];;
...
let rnd a b = Random.int (b - a + 1) + a

let shuffle_list l =
  let temp = List.map (fun x -> Random.bits (), x) l in
...
let hash_label n =
  let rec aux acc n =
    ...
    then acc
      ...
      let l = n lxor rand_const mod 256 in
